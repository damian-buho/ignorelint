...
- `--fix` auto-corrects nine deterministic codes (`IG-001,002,003,008,015,018,022,023,024`): line replacements/deletions are applied first, then a bulk `SortFix` alphabetises all active lines; fixed issues are relabelled severity `fixed` and excluded from the fail decision.
