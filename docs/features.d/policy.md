...
- Lint policy (`fail-on`, `format`, `fix`, `disabled-rules`, `override`) lives in the `org.ignorelint` subtree of `projectfile.yaml`, so one file carries project identity and lint rules together. See the [CLI reference](docs/cli.md).
